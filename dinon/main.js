...
const cloudsElement2 = document.querySelector(".clouds2")
...
setTimeout(function() {
    cloudsElement2.style.display = ".clouds2";
}, 4000);
...
function init() {
    ...
    for(let i = 0; i < 5; i++){
        ...
        foodImage.src = Math.random() > 0.5 ? "cactus.png" : "pokeball.png";
...
function update() {
    ...
    world.objects.clouds.forEach((cloud) => {
        ...
        if (cloudRect.left + cloudRect.width - 20 <= cloudsRect.left){
            ...
            cloud.scale = getRandomNumber(.7,1.4);
        } else {
